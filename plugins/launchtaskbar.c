...
#define taskbar_reset_menu(tb) \
    if (tb->tb_built)          \
    task_button_reset_menu(tb->tb_icon_grid)
                                            ...
                                            LaunchTaskBarPlugin *lb)
{
    ...
    case LAUNCHBUTTON_DND_TARGET:
        if (panel_icon_grid_get_dest_at_pos(ig, x, y, &btn, &pos) &&
            PANEL_IS_LAUNCH_BUTTON(btn) && (pos != PANEL_ICON_GRID_DROP_INTO || launch_button_get_settings((LaunchButton *)btn) == NULL))
        {
...
static void launchbar_configure_move_up_button(GtkButton *widget, LaunchTaskBarPlugin *ltbp)
{
    ...
    if (gtk_tree_selection_get_selected(gtk_tree_view_get_selection(defined_view), &list, &it))
    {
        ...
        if ((gtk_tree_path_get_indices(path)[0] > 0) && (gtk_tree_path_prev(path)))
        {
            GtkTreeIter it2;
    ...
    if (client_list != NULL)
    {
        ...
        for (i = 0; i < client_count; i++)
        {
            ...
            if (l == NULL)
            {
                ...
                if ((accept_net_wm_state(&nws)) && (accept_net_wm_window_type(&nwwt)))
                {
