...
static gboolean dclock_update_display(DClockPlugin *dc)
{
  ...
  if ((!dc->icon_only) && ((dc->prev_clock_value == NULL) || (strcmp(dc->prev_clock_value, clock_value) != 0)))
  {
  ...
  if (dc->expiration_interval < ONE_SECOND_INTERVAL)
  {
    ...
    {
      if (((dc->icon_only) || (strcmp(dc->prev_clock_value, clock_value) == 0)) && (strcmp(dc->prev_tooltip_value, tooltip_value) == 0))
      {
