...
static gboolean task_set_names(TaskDetails *tk, Atom source)
{
    ...
    if ((name == NULL) && ((source == None) || (source == a_NET_WM_NAME)) && ((tk->name_source == None) || (tk->name_source == a_NET_WM_NAME) || (tk->name_source == XA_WM_NAME)))
    {
    ...
    if ((name == NULL) && ((source == None) || (source == XA_WM_NAME)) && ((tk->name_source == None) || (tk->name_source == XA_WM_NAME)))
    {
...
static void _task_update_icon(TaskButton *task, TaskDetails *details, Atom source)
{
    ...
    if ((pixbuf == NULL) && ((source == None) || (details->image_source == None)))
    {
...
gboolean task_button_window_reconfigured(TaskButton *button, Window win)
{
    ...
    if (button->flags.same_monitor_only && (old_mon == button->monitor || new_mon == button->monitor))
    {
        details->visible = task_is_visible(button, details);
        task_update_visibility(button);
        task_redraw_label(button);
