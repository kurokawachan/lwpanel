...
static gboolean icon_grid_placement(IconGrid *ig)
{
  ...
  for (ige = ig->child_list; ige != NULL; ige = ige->flink)
  {
    if (ige->visible)
    {
      ...
      if (((child_width != allocation.width) || (child_height != allocation.height)) && (child_width > 0) && (child_height > 0))
      {
        GtkAllocation alloc;
        alloc.x = x;
        alloc.y = y;
...
static void icon_grid_geometry(IconGrid *ig, gboolean layout)
{
  ...
  if ((layout) && ((!ig->actual_dimension) || (ig->rows != original_rows) || (ig->columns != original_columns) || (ig->container_width != allocation.width) || (ig->container_height != allocation.height) || (ig->children_changed)))
  {
